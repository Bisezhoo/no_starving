...
|------|------|
| 文档版本 | v0.29 |
| 最近更新时间 | 2026-06-14 20:17:00 CST |
| 文档状态 | ✅ 技术设计已确认，前端展示层级修正已实现，Prompt 外部配置化与前端交互优化已实现 |
...
| `AssistantMessage.vue` | 单条 Agent 消息展示：文本回复直出，按结构化数据决定是否渲染结果区块 |
| `AssistantResultBlock.vue` | 结构化结果区块：仅在 `cards` 非空时展示卡片，并按需展示食材/步骤、Tool 调用和画像变化辅助视图 |
...
9. `taste-profile.json`、`recommendation-history.json` 和 `agent-memory.json` 共用同一个可靠写入路径：写入目标同目录临时文件，`flush` 后对临时文件执行 `fsync`，再通过原子 `rename` 覆盖目标文件，最后对父目录执行 `fsync`，降低进程崩溃或断电导致半写、空文件或目录项未落盘的风险。
10. 临时文件命名固定在目标文件同目录内，失败残留不参与读取；后续写入可覆盖同名临时文件，不在后端执行文件删除操作。
...
| `AssistantMessage` / `AssistantResultBlock` | 无卡片时只展示文本和警告，不渲染推荐容器；有卡片时文本与结果区同时保留；`done.warnings` 展示为非阻塞提示，不清空已展示回复和卡片 |
...
| C3 | 页面刷新后是否保留画像和上下文 | 使用后端本地 JSON 保留单用户轻量画像、推荐历史和 Agent Memory |
