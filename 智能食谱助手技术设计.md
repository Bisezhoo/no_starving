...
|------|------|
| 文档版本 | v0.36 |
| 最近更新时间 | 2026-06-14 21:53:33 CST |
| 文档状态 | ✅ 技术设计已确认，前端展示层级修正、第三方 Markdown 文本渲染、交互优化、最近一天完整聊天记录加载、推荐结果默认收起和流式思考占位已确认 |
...
| `ChatPage.vue` | 页面容器，维护消息列表、发送状态和本轮 assistant `pending` 展示状态 |
...
| `ChatPage` | 页面挂载时加载最近 24 小时、最多 60 条历史消息；加载失败不阻断发送 |
