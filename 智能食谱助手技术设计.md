...
| 文档状态 | ✅ 技术设计已确认，前端展示层级修正、第三方 Markdown 文本渲染、交互优化、最近一天完整聊天记录加载、推荐结果默认收起、重查 Tool 结果前置、流式思考占位、空聊天默认欢迎语、全页面固定聊天布局、新消息自动滚动、流式开始顶部思考提示、Tool 调用默认折叠、卡片详情弹窗和 Tool 结果统一翻译校验已确认 |
...
1. 后端承载全部业务逻辑，包括参数校验、语言检测、Tool 调用、外部字段清洗、画像合并、推荐排序、卡片本地化翻译校验和错误降级。
...
9. Tool 返回值不直接翻译；卡片本地化由后端聚合层统一完成，LLM 只返回 `localized*` 补丁，校验通过后合并，失败时回退原始卡片。
...
flowchart LR
  ...
  Recommend --> Translate["CardTranslationService 本地化翻译与校验"]
  Translate --> Agent
  Agent -->|SSE events| UI
...
| `services/card_translation_service.py` | 在卡片发送前统一生成、校验并合并本地化补丁；保留原始字段，翻译失败时降级 |
...
| `ChatPage.vue` | 页面容器，维护消息列表、发送状态、本轮 assistant `pending` / `streaming` 展示状态和空聊天默认欢迎语 |
| `MessageList.vue` | 消息列表，按消息角色分发渲染，并在历史加载、新消息插入和流式内容更新后自动滚动到底部 |
| `MessageComposer.vue` | 输入框、发送按钮、空输入和长度提示；Enter 发送消息，Ctrl+Enter / Cmd+Enter 换行；发送中按钮显示旋转加载图标（Loader2）并禁用 |
| `AssistantMessage.vue` | 单条 Agent 消息展示：`pending` 时展示“思考中”，`streaming` 时在顶部展示“疯狂思考中，请稍等”，文本回复直出，按结构化数据决定是否渲染结果区块 |
| `AssistantResultBlock.vue` | 结构化结果区块：仅在 `cards` 非空时展示卡片；超过 3 个推荐时默认展示前 3 个，展开后用横向滑动轨道展示全部，并按需展示食材/步骤、Tool 调用和画像变化辅助视图 |
| `MealCard.vue` | 菜谱卡片展示，不解析外部原始字段；优先展示后端返回的 `localized*` 字段 |
| `CocktailCard.vue` | 饮品卡片展示，不解析外部原始字段；优先展示后端返回的 `localized*` 字段 |
| `ToolTracePanel.vue` | 展示 Tool 调用链路；默认收敛为 `Loaded N tools` 小字状态，点击后展开每次调用明细 |
| `ProfilePanel.vue` | 展示本轮画像变化 |
| `styles/app.css` | 控制全页面三段式布局：顶部标题区贴页面顶部，底部输入区贴页面底部，中间消息列表独立滚动 |
前端不做：意图识别、食材翻译、卡片翻译、推荐排序、画像合并、画像持久化、聊天历史裁剪、权限控制、外部 API 直连。
...
1. `.app-shell` 固定为视口高度并隐藏页面级滚动。
2. `.chat-surface` 使用 `grid-template-rows: auto minmax(0, 1fr) auto`，让顶部标题区和底部输入区占用自身高度，中间消息区吃掉剩余高度。
3. `.message-list` 设置 `min-height: 0` 和 `overflow-y: auto`，聊天记录只在中间区域纵向滚动。
4. 桌面端只保留左右 24px 外边距；上下方向不保留外边距，使标题区和输入区贴住整个页面的上边缘和下边缘。
5. 移动端继续全宽全高展示。

### 5.5 新消息自动滚动

`MessageList.vue` 持有 `.message-list` DOM 引用，并把自动滚动限定在消息列表组件内部：

1. 组件挂载后执行一次 `scrollToBottom()`，让历史消息或默认欢迎语进入页面后位于底部。
2. 使用深度 `watch` 监听 `messages`，并设置 `flush: "post"`，确保 Vue 完成 DOM 更新后再滚动。
3. `scrollToBottom()` 将 `scrollTop` 设置为 `scrollHeight`，覆盖用户发送消息、assistant 占位消息、流式 `delta` 文本增长、卡片/错误/Tool 轨迹更新等展示变化。
4. 本次不增加用户手动上滑时暂停自动滚动的策略；如后续需要阅读历史消息体验，可单独设计“仅接近底部时自动滚动”。

### 5.6 流式开始顶部思考提示

前端将本轮 assistant 消息区分为两个 UI 状态：

1. `pending=true`：用户已发送消息，但还没有确认后端流式响应开始，用于显示首屏占位“思考中”。
2. `streaming=true`：已收到 SSE `meta` 事件，表示后端流式返回开始但本轮尚未完成，用于在当前助手消息顶部显示“疯狂思考中，请稍等”。

`streaming` 是前端运行时 UI 字段，不写入 `chat-history.json`，也不属于后端 API 契约。收到 `done`、`error` 或 `streamChat()` 抛出异常后，前端必须把 `streaming` 置回 `false`。收到 `delta`、`card`、`tool_call`、`tool_result` 或 `profile_update` 时，`streaming` 保持 `true`。
...
2. 收到 `meta` 时，前端清除 `pending`，设置 `streaming=true`，并在助手消息顶部展示“疯狂思考中，请稍等”。
3. 收到 `delta`、`card`、`tool_call`、`tool_result` 或 `profile_update` 时，前端保留 `streaming=true`，让顶部提示与正文或卡片同时存在。
4. 收到最终 `done` 或 `error` 时，前端清除 `pending` 和 `streaming`，展示实际回复或错误。
5. SSE 请求建立失败时，前端清除 `pending` 和 `streaming`，并展示“连接失败，请重试”或后端 JSON 错误。
6. SSE 中途断开且未收到 `done` 时，前端保留已收到内容，清除 `pending` 和 `streaming`，并展示“生成中断，请重试”。
7. 不自动改走非流式接口，避免重复 Tool 调用、重复画像更新和幂等复杂度。
8. 自动化测试直接测试 `AgentOrchestrator`、Tool、领域函数和 SSE 事件解析，不依赖非流式 HTTP 接口。
9. 如果后续部署环境证明 SSE 被网关或代理缓冲，再评估新增非流式接口；新增时必须复用同一个 Agent 事件生成器，不允许复制业务逻辑。
...
`card` 事件和 `done.cards` 必须使用同一个统一结构：`{ cards: Card[] }`。后端负责把菜谱和饮品都序列化为带 `type` 的 `Card` 联合类型，并在发送前合并已通过校验的 `localized*` 字段；前端只根据 `type` 分发到 `MealCard.vue` 或 `CocktailCard.vue`，不自行猜测卡片来源。
...
8. Tool 调用轨迹由 `ToolTracePanel.vue` 统一默认折叠为 `Loaded N tools` 摘要；点击摘要才展示 Tool 名称、状态、结果数、耗时和错误信息，再次点击收起。
9. 同一轮 Agent 多次产生可展示卡片时，后端必须在 `AgentOrchestrator` 聚合阶段把本次 Tool 查询新增候选放到 `cards` 最前面；旧候选保留在后，重复 ID 只保留首次展示的一张。前端不得自行按 Tool 时间重排，只按后端 `cards` 顺序展示。
...
| `localizedIngredients` | 与 `ingredients` 同结构的本地化副本；数组长度和字段类型必须保持一致 |
...
Tool 标准化层仍只负责清洗外部 API 原始字段，不直接翻译；`AgentOrchestrator` 在推荐排序、去重和新批次前置完成后，调用 `CardTranslationService` 统一处理准备展示的 `Card[]`。处理结果必须在进入 `card` 事件和 `done.cards` 前合并完成。

执行顺序：

```text
ToolResult -> Normalizer -> rank_recommendations -> merge_new_batch_first -> CardTranslationService -> SSE card/done
```
...
| `localizedTitle` | 可选辅助展示，不替代 `title`；缺失时前端展示 `title` |
| `localizedSummary` | 列表和详情卡片都可生成；按本轮输出语言解释类型、来源、核心食材/配料和推荐理由 |
| `localizedIngredients` | 与 `ingredients` 一一对应，只翻译 `name` / `measure` 的展示文本，不改变数组长度和结构 |
| `localizedInstructions` | 仅当用户本轮询问做法、步骤、详情或 how-to 意图时生成；不覆盖 `instructions` |
| `localizedCategory` / `localizedCountry` | 菜谱分类和地区的本地化辅助展示；缺失时回退原字段 |
| `localizedCategory` / `localizedAlcoholic` / `localizedGlass` | 饮品分类、酒精类型和杯型的本地化辅助展示；缺失时回退原字段 |

翻译调用策略：

1. `CardTranslationService` 接收最终准备展示的卡片批次、`detectedLocale` 和本轮详情意图标记。
2. 当待翻译源内容语言与 `detectedLocale` 一致时，不调用翻译 LLM；例如英文卡片面向英文输出时直接使用原始字段或模板摘要。
3. 一轮 Chat 不限制只能调用一次翻译 LLM。服务可按新增卡片批次、payload 长度或详情步骤长度拆分多次调用，但每次调用都必须独立校验和可降级。
4. 翻译调用不参与 Tool 参数修正次数，也不计入 ReAct `maxLlmSteps`，因为它不参与 Agent 决策循环；但必须受单轮 Chat 总超时约束，超时后丢弃未完成翻译并继续返回原始卡片。
5. 翻译失败、返回非法 JSON 或结构校验失败都不阻断主回复、Tool 结果、画像更新和推荐历史写入。

LLM 只能返回本地化补丁，不能返回完整卡片：

```ts
type CardTranslationPatch = {
  id: string;
  localizedTitle?: string;
  localizedSummary?: string;
  localizedIngredients?: IngredientItem[];
...
1. `id` 必须存在于输入卡片集合中，同一卡片同一批次最多合并一个补丁。
2. 禁止未知字段；补丁中不得出现 `title`、`ingredients`、`instructions`、`matchReasons`、`sourceUrl`、`youtubeUrl` 等原始或事实字段。
3. `localizedIngredients` 必须与原始 `ingredients` 数组长度一致；每一项只允许 `name` 和可选 `measure`，字段类型必须为字符串。
4. `localizedInstructions` 仅在原卡片存在 `instructions` 且本轮需要详情说明时允许出现；数组元素必须为字符串。
5. 所有本地化字符串必须 `trim` 后非空，并受长度限制；超长字段按校验失败处理，不做静默截断。
6. 单张卡片补丁校验失败时只丢弃该卡片补丁；其他合法补丁可以继续合并。
7. 合并只写入 `localized*` 字段和 `localizedLanguage`，不得修改排序、候选 ID、推荐历史去重依据或 Tool 调用参数。

前端展示优先级：

| 展示内容 | 优先级 |
|----------|--------|
| 标题 | `localizedTitle ?? title`，必要时可在小字保留原英文名 |
| 摘要 | `localizedSummary`，缺失时不由前端生成 |
| 食材/配料 | `localizedIngredients ?? ingredients` |
| 做法步骤 | `localizedInstructions ?? instructions` |
| 分类/地区/酒精/杯型 | 对应 `localized* ?? 原字段` |
...
10. 同一轮多次 Tool 查询产生的展示顺序以“最新有效查询批次优先”为准：每批内部仍使用 `rank_recommendations()` 排序，批次之间新批次整体前置，避免默认只显示前三张时旧候选遮挡重新查询后的更相关结果。
...
| 卡片详情弹窗 | 用户点击单张卡片的“查看详情”按钮 | 当前单个菜谱或饮品的食材/配料、用量和做法 |
...
6. `error` 展示错误文案并恢复输入。
7. 菜谱和饮品卡片默认只展示概要信息；完整食材、配料和步骤不在结果区下方直接平铺，只在当前卡片详情弹窗中展示。
8. 弹窗详情优先展示 `localizedInstructions`；没有本地化步骤时回退外部 API 原始 `instructions`。
9. 卡片展示优先使用后端返回的 `localized*` 字段；缺失时回退原始字段，前端不得自行生成翻译或改写卡片数据。
...
| 卡片翻译失败或补丁校验失败 | 丢弃失败补丁，保留原始卡片和已有合法 `localized*` 字段；可在 `done.warnings` 追加非阻塞提示 | 卡片正常展示，缺失本地化字段时回退原始字段 |
...
| 卡片翻译 | `detectedLocale`、卡片数量、翻译调用次数、跳过数量、补丁成功数量、补丁失败数量、耗时、失败原因摘要 |
...
| `card_translation_service.py` | 源/目标语言一致时跳过 LLM；多批次翻译调用；只接受 `localized*` 补丁；未知字段、数组长度不一致、类型错误和超长字段均拒绝；单卡补丁失败不影响其他卡片 |
...
| 中文卡片本地化 | 中文提问返回中文 `localizedTitle`、`localizedSummary`、`localizedIngredients` 和必要 `localizedInstructions`，同时保留英文原始字段 |
| 英文卡片跳过翻译 | 英文提问且源内容为英文时不调用翻译 LLM，仍能正常返回卡片 |
| 翻译补丁失败降级 | 翻译 LLM 返回非法 JSON、未知字段或长度不一致数组时，后端丢弃非法补丁并返回原始卡片 |
...
| `ChatPage` | 页面挂载时加载最近 24 小时、最多 60 条历史消息；历史为空或加载失败时展示默认欢迎语；收到 `meta` 后展示流式开始顶部提示，`done`/`error`/异常后隐藏；加载失败不阻断发送 |
| `MessageComposer` | 空消息禁发、长度限制、发送中禁用 |
| `AssistantMessage` / `AssistantResultBlock` | 无卡片时只展示文本和警告，不渲染推荐容器；发送后无回复无错误时展示“思考中”；收到 `meta` 后在消息顶部展示“疯狂思考中，请稍等”；`done`/`error`/异常后隐藏顶部提示；有卡片时文本与结果区同时保留；超过 3 个推荐时默认只显示前 3 个，点击“查看更多”后以横向滑块展示全部并可“收起”；Markdown 通过 `marked + DOMPurify` 渲染并清洗危险 HTML；`done.warnings` 展示为非阻塞提示，不清空已展示回复和卡片 |
| `MealCard` / `CocktailCard` | 缺失字段不崩溃，不显示空标签；优先展示 `localizedTitle`、`localizedSummary`、`localizedIngredients` 和本地化分类/地区/酒精/杯型字段 |
...
| Phase 2 | FastAPI 流式 Chat、Agent 编排、卡片本地化翻译、SSE 事件、错误降级、`conversation_lock`、本地 JSON 持久化 | 能完成单轮和多轮对话，支持 `localized*` 字段、`done.warnings`，客户端断开后释放锁 |
...
| 序号 | 任务 | 状态 | 备注 |
|------|------|------|------|
| 1 | 读取 `智能食谱助手需求分析.md` | ✅ 已完成 | 已确认当前关联需求版本 v0.37 |
...
| 5 | 设计 API、SSE 和数据模型 | ✅ 已完成 | 已覆盖首版流式接口、统一 Card[]、画像/推荐历史/Agent Memory 边界、本地化字段与翻译补丁边界、数据清洗边界、推荐硬/软边界、Tool 英文参数归一化和前置 HTTP 错误边界 |
| 6 | 设计错误处理、日志和测试方案 | ✅ 已完成 | 已覆盖 ToolRunner、参数修正防幻觉、外部 API、主查询优先级、Tool `limit` 语义、OpenRouter、画像损坏、JSON 写入失败、`done.warnings`、客户端断开、敏感日志启动开关、卡片翻译降级、推荐兜底和前端状态 |
...
| 17 | 确认全页面固定聊天布局 | ✅ 已完成 | v0.38 已确认顶部标题区贴页面顶部、底部输入区贴页面底部、中间消息列表独立滚动 |
| 18 | 确认新消息自动滚动到底部 | ✅ 已完成 | v0.39 已确认自动滚动归属 `MessageList.vue`，覆盖历史加载、新消息和流式内容更新 |
| 19 | 确认流式开始顶部思考提示 | ✅ 已完成 | v0.40 已确认收到 SSE `meta` 后显示“疯狂思考中，请稍等”，`done`/`error`/异常后隐藏 |
| 20 | 确认 Tool 结果统一翻译与结构校验设计 | ✅ 已完成 | v0.44 已确认新增 `CardTranslationService`，LLM 只返回本地化补丁，校验失败回退原始卡片 |
...
| v0.38 | 2026-06-14 22:43:54 CST | 根据用户确认新增全页面固定聊天布局：顶部标题区贴页面顶部，底部输入区贴页面底部，中间消息列表独立滚动 |
| v0.39 | 2026-06-14 22:55:53 CST | 根据用户确认新增新消息自动滚动到底部：`MessageList.vue` 在挂载和消息变化后滚动到 `scrollHeight` |
| v0.40 | 2026-06-14 23:00:15 CST | 根据用户确认新增流式开始顶部思考提示：收到 SSE `meta` 后在当前助手消息顶部显示“疯狂思考中，请稍等”，流式完成或报错后隐藏 |
| v0.43 | 2026-06-14 23:46:14 CST | 根据用户反馈新增同一轮重查 Tool 结果前置规则：`AgentOrchestrator` 聚合新批次卡片时整体前置，前端继续按后端顺序展示 |
| v0.44 | 2026-06-15 00:01:55 CST | 根据用户确认新增 Tool 结果统一翻译设计：`CardTranslationService` 在卡片发送前生成并校验 `localized*` 补丁，支持一轮多次翻译调用，同语言跳过，失败回退原始卡片 |
